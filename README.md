...
- **精選實用範本庫**：內建 5 款高質感小工具（番茄鐘、隨機決策轉盤、多色便箋待辦、極簡塗鴉白板、標準計算機），一鍵即刻加入看板。
- **就地編輯代碼與尺寸**：卡片支援就地更新名稱、尺寸寬度（1x/2x）與代碼內容，並提供即時預覽。
- **即時搜尋與類型過濾**：提供關鍵字搜尋與「自訂程式 / Iframe」即時分類過濾。
- **空間備份與移轉 (JSON)**：支援將整套空間工具一鍵匯出為 JSON 備份檔，並可上傳匯入復原。
- **統一帳號體系**：純粹的帳號密碼註冊與登入，無身分限制，維護個人與團隊數據隔離。
