...
- **就地編輯代碼與尺寸**：卡片支援就地更新名稱、標籤、尺寸寬度（1x/2x）與代碼內容，並提供即時預覽。
- **空間備份與移轉 (JSON v2.0)**：支援將整套空間工具（含標籤與置頂狀態）一鍵匯出為 JSON 備份檔，並可上傳匯入復原。
- **多空間與邀請碼共享**：可建立多個專案空間，並透過「空間邀請碼 (`SPC-XXXX`)」邀請團隊成員加入共享。
- **多元閱讀佈局**：支援「網格並列」、「分頁切換」與「折起專注模式（全部只顯示名稱，點開就放大）」。
